...
class LivenessController {
  @Get('health/live')
  live() {
    return { status: 'alive', runtime: 'edge', capability: 'batch-1-liveness-only' };
  }
}
...
async function bootstrap(): Promise<void> {
  const config = loadConfig('edge', process.env);
  // Framework logging is disabled until the scoped structured logging task exists.
  const app = await NestFactory.create<NestFastifyApplication>(EdgeRoot, new FastifyAdapter({ logger: false }), { logger: false });
  let stopping = false;
  const stop = async () => {
    if (stopping) return;
    stopping = true;
...
void bootstrap().catch(error => {
  ...
  process.stderr.write(JSON.stringify({ event: 'runtime.failed', runtime: 'edge', code }) + '\n');
  process.exitCode = 1;
